...
int divide_by_10(uint64_t temp[6]) {
  uint64_t remainder = 0;
  // Проходим по частям числа от старшей к младшей
  for (int i = 5; i >= 0; i--) {
...
int s21_mul(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  *result = (s21_decimal){{0, 0, 0, 0}};
  ...
  while ((temp[3] || temp[4] || temp[5]) && res_scale > 0) {
    int remainder = divide_by_10(temp);

    if (remainder > 5) {
        add_one(temp);
    } else if (remainder == 5) {
        if (temp[0] & 1) {  // если нечетное, то округляем вверх
            add_one(temp);
        }
    }
    res_scale--;
...
//   int status = s21_mul(e, f, &res3);

//   printf("Статус умножения: %d\n", status);

//   s21_print_decimal(res3);
  
//   return 0;
// }

// int main() {
//   uint32_t scale = (uint32_t) 14 << 16;
//   s21_decimal a = {{0x860DDF79, 0x00007048, 0, scale}};
//   float result = 0;
