...
int s21_is_greater(s21_decimal a, s21_decimal b) {
    ...
    if (sign_a != sign_b) {
        result = sign_a < sign_b ? 1 : 0;  // Положительное число всегда больше отрицательного
    } else {
        if (sign_a == 1) {  // Оба числа отрицательные — сравниваем наоборот
            for (int i = 2; i >= 0 && !flag; i--) {
                if (tmp1.bits[i] < tmp2.bits[i]) {  // Инверсия сравнения
                    result = 1;
                    flag = 1;
        ...
        } else {  // Обычное сравнение для положительных чисел
            for (int i = 2; i >= 0 && !flag; i--) {
...
//     //printf("%d\n", res_code); // 0
//     printf("%d %d\n", result.bits[0], expected.bits[0]); // 12193262
//     printf("%d %d\n", result.bits[1], expected.bits[1]); // 0
//     printf("%d %d\n", result.bits[2], expected.bits[2]); // 0
//     printf("%d %d\n", result.bits[3], expected.bits[3]); 
//     printf("%d %d\n", s21_get_scale(result), s21_get_scale(expected)); // 7
